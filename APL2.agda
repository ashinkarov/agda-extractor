...
_^^_ : ∀ {X Y : Set} → (X → Y) → List X → List Y
f ^^ A = L.map f A

⦅_⦆_^^_ : ∀ {X Y Z : Set} → List X → (X → Y → Z) → List Y → List Z
⦅ a ⦆ f ^^ b = L.zipWith f a b
...
module reduce-custom where
  ...
  sh? hole = do
    goal ← inferType hole
    q ← reduce goal
    typeError (strErr "test: " ∷ termErr q ∷ [])
    {-
    reduce m >>= λ where
      (lit (nat 0)) → unify hole (con (quote 0-n) [])
      (meta id _) → blockOnMeta id
...
infixr 20 _+_ _×_
_+_ : ∀ {A B d s}{{t : DyScalVec ℕ A B d s}} → A → B → Ar ℕ d s
a + b = lift a b N._+_
...
--infixr 20 _-_
--_-_ : ∀ {d s} → (a b : Ar ℕ d s) → .{≥ : a ≥a b} → Ar ℕ d s
--_-_ (imap a) (imap b) = imap λ iv → a iv  N.∸ b iv
